 ...
 * 1、SwipeItemLayout拷贝到项目里
 ...
 至于item布局方面，与DrawerLayout一样的做法。以SwipeItemLayout为root view，然后添加一个group，作为主要内容，
 再添加第二个group，作为菜单按钮部分。点击事件、长按事件以及view的点击效果等，
 将item中的children当做一个普通的view来对待就可以了。直接setOnClickListener、setOnLongClickListener，
 点击效果用drawable设置background即可。
 *
 *
 * @author 唐浩*/
