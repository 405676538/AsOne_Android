        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mActivePointerId == INVALID_POINTER) {
                    Log.e(LOG_TAG, "Got ACTION_MOVE event but don't have an active pointer groupId.");
                    return false;
                }

                final int pointerIndex = MotionEventCompat.findPointerIndex(ev, mActivePointerId);
                if (pointerIndex < 0) {
                    Log.e(LOG_TAG, "Got ACTION_MOVE event but have an invalid active pointer groupId.");
                    return false;
                }
...
//                final float yDiff = y - mInitialMotionY;
                ...
                //下拉
                if (yDiff > mTouchSlop && canRefresh)
                {
